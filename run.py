...
def viz_main(combined_dataframe,title,x_axis,y1_label,
            agg_on='timestep',groupby_stat='mean',y2_label=None):
    '''
    ...
    agg_df = combined_dataframe#.groupby(combined_x_axis).agg(agg_files_stat).reset_index()
    ...
    if y2_label is not None:
        plt.ylim(min(agg_df[y1_label].min(),agg_df[y2_label].min()),
...
def data_exploration():
    ...
    #sub_df.plot.scatter(x='timestep', y='lame_0')
    #plt.show()

    #viz_main(sub_df,title='Lame Parameters',x_axis='timestep',y1_label='lame_1',y2_label='lame_0')
    #pandas_scatter_all_cols(sub_df, y_axis='lame_0')
